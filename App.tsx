...
const BreakingNewsTicker: React.FC = () => {
    ...
    useEffect(() => {
        const fetchBreaking = async () => {
            ...
            // Use mock data only for the ticker if it's completely empty to maintain layout
...
const AppContent: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const loadNews = async () => {
            setIsFetching(true);
            setArticles([]); // Start fresh to avoid ghost cards
    ...
    const filteredArticles = useMemo(() => {
        ...
        // For Azad Studio: If empty, return EMPTY array. 
        // This ensures NO cards show up if RSS fails, leaving only the iframes.
        if (selectedCategory === 'azad-studio') {
            return filtered; 
        }

        // For other categories: If empty, show fallback content to avoid blank screens
            ...
            default:
                return (
                    <div className="p-4 md:p-8 max-w-7xl mx-auto">
                        ...
                        {selectedCategory === 'azad-studio' && (
                             <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
                                <div className="lg:col-span-2 bg-neutral-900 rounded-2xl overflow-hidden border border-amber-900/30 shadow-2xl h-[500px] md:h-[600px] relative group">
                                    ...
                                    {/* Embed Mini App Deep Link / Placeholder */}
                                    <div className="w-full h-full relative">
                                        <iframe 
                                            src="https://t.me/s/AzadStudioOfficial?embed=1" 
                                            className="w-full h-full border-0 mt-5 bg-neutral-900 absolute inset-0 z-0"
                                            title="Azad Studio Telegram"
                                        ></iframe>
                                        {/* Overlay Button for Mini App */}
                                        <div className="absolute bottom-4 right-4 z-20">
                                            <a 
                                                href="http://t.me/AzadStudioBot/Newspulse" 
                                                target="_blank" 
                                                rel="noopener noreferrer"
                                                className="flex items-center gap-2 bg-[#24A1DE] text-white px-4 py-2 rounded-full font-bold text-xs shadow-lg hover:bg-[#2095cf] transition-all"
                                            >
                        ...
                        {/* Article Grid - Will be empty for Azad Studio if no new articles */}
